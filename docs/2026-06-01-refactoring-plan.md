...
| 颜色配置迁移到 `constants.ts` | — | `GROUP_COLORS` 集中管理 |
...
| # | 问题 | 方案 |
|---|------|------|
| 8 | `console.log` 留在生产代码 | `useGroups.ts`（8 处）、`GroupLayout.tsx`、`ShortcutGroupCard.tsx` 中的 debug log 全部清理 |
| 9 | `GroupDialog.tsx` 和 `ShortcutGroupCard.tsx` 颜色硬编码重复 | 统一提取到 `constants.ts` 的 `GROUP_COLORS` |
